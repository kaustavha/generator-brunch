            ...
            switch line.charAt(spaces)
                when tabChar
                    while line.charAt(spaces) is '	' 
                        spaces += 1
                        if spaceStr? then spaceStr += tabChar else spaceStr = tabChar
                when ' '
                    while line.charAt(spaces) is ' '
                        spaces += 1
                        if spaceStr? then spaceStr += ' ' else spaceStr = '  '
            fs.appendFileSync tmpFile, args.splicable.map (line) ->
                spaceStr + line + '\n'
